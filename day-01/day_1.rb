...
def day_one(calibration_values)
  coordinate_sum = 0

  calibration_values.each do |calibration_value|
    left_most_number = find_left_most_number_as_char(calibration_value)
    continue if left_most_number.nil?

    right_most_number = find_right_most_number_as_char(calibration_value)

    coordinate = (left_most_number + right_most_number).to_i
    coordinate_sum += coordinate
  end

  puts "\nCoordinate Sum: #{coordinate_sum}"
end

# Return either a char or nil
def find_left_most_number_as_char(calibration_value)
  char_index = calibration_value.length
  char_value = nil
  word_index = calibration_value.length
  word_value = nil

  for index in (0..calibration_value.length - 1)
    char = calibration_value[index]
  ...
  NUMBER_MAP.each_key do |key|
    index = calibration_value.index(key.to_s)
    if !index.nil? && index < word_index
      word_index = index
      word_value = NUMBER_MAP[key]
    end
  end
...
def find_right_most_number_as_char(calibration_value)
  char_index = -1
  char_value = nil
  word_index = -1
  word_value = nil

  revesed_calibration_value = calibration_value.reverse
  for index in (0..calibration_value.length - 1)
    char = revesed_calibration_value[index]
    next unless number?(char)

    char_index = calibration_value.length - (index + 1)
    char_value = char
    break
  end

  NUMBER_MAP.each_key do |key|
    index = calibration_value.rindex(key.to_s)
...
def number?(char)
  # ASCII range for digits is 48-57
  char.ord >= 48 && char.ord <= 57
end

day_one(calibration_values)

# answer to #1 is 54081
# answer to #2 is 54649
